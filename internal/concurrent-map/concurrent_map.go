...
func (m *ConcurrentMapShared[K, V]) UnsafeSet(key K, value V) {
	m.items[key] = value
}
func (m *ConcurrentMapShared[K, V]) UnsafeGet(key K) (V, bool) {
	v, ok := m.items[key]
	return v, ok
}
...
func (m ConcurrentMap[K, V]) GetShardIndex(key K) uint {
	return uint(m.sharding(key)) % uint(SHARD_COUNT)
}
func (m ConcurrentMap[K, V]) GetShardByIndex(idx uint) *ConcurrentMapShared[K, V] {
	if idx >= uint(SHARD_COUNT) {
		log.Fatal("Overflow")
	}
	return m.shards[idx]
}
...
type GetCb[K any, V any] func(key K, v V, exists bool) (V, bool)

// Get retrieves an element from map under given key.
func (m ConcurrentMap[K, V]) GetCb(key K, cb GetCb[K, V]) (V, bool) {
	// Get shard
	shard := m.GetShard(key)
	shard.RLock()
	// Get item from shard.
