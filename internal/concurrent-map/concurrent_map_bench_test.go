...
func BenchmarkMultiInsertDifferentSyncMap(b *testing.B) {
	var m sync.Map
	finished := make(chan struct{}, b.N)
	_, set := GetSetSyncMap[int, string](&m, finished)
...
func BenchmarkMultiInsertDifferentLockMap(b *testing.B) {
	lock := sync.RWMutex{}
	var m map[int]string = make(map[int]string, b.N)
	finished := make(chan struct{}, b.N)
	_, set := GetSetLockMap(&lock, m, finished)
	//_, set := GetSetSyncMap[int, string](&m, finished)
...
func GetSetLockMap[K comparable, V any](lock *sync.RWMutex, m map[K]V, finished chan struct{}) (get func(key K, value V), set func(key K, value V)) {
	get = func(key K, value V) {
		for i := 0; i < 10; i++ {
			lock.RLock()
			_ = m[key]
